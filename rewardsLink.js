...
// Function to build dynamic rewards message
function buildRewardsMessage(restaurantData) {
  const restaurantName = restaurantData.restaurantName || 'Restaurant';
  
  // Base greeting
  let message = `Thank you for your interest in the ${restaurantName} Rewards Program!`;
  
  // Get rewards link
  const rewardLink = restaurantData.rewardLink;
  
  // Add rewards lookup section if link exists
  if (rewardLink) {
    message += ` To look up your rewards account, please visit us at ${rewardLink}`;
  } else {
    // Fallback if no rewards link available
    message += ` Please call us directly to look up your rewards account information.`;
  }
...
// Function to send rewards links via SMS
module.exports.sendRewardsLink = async (event) => {
  console.log('Processing rewards link SMS request...');
  ...
  try {
    ...
    console.log(`Processing rewards links for location: ${locationId}, customer: ${customerPhone}`);
    ...
    const dynamicMessage = buildRewardsMessage(restaurantData);

    // Send SMS with dynamic message
    const smsResult = await sendRewardsLinkSMS(customerPhone, dynamicMessage, restaurantData.restaurantName);
    console.log('✅ Dynamic rewards links SMS sent successfully');

    return createSuccessResponse({
      success: true,
      message: 'Rewards links sent successfully',
      restaurantName: restaurantData.restaurantName,
      smsResult: smsResult
    });

  } catch (error) {
    console.error('Error in rewards link workflow:', error.message);
...
// Function to send rewards links SMS using TextBelt
async function sendRewardsLinkSMS(phoneNumber, message, restaurantName) {
