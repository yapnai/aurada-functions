...
// Function to build dynamic catering message
function buildCateringMessage(restaurantData) {
  const restaurantName = restaurantData.restaurantName || 'Restaurant';
  
  // Base greeting
  let message = `Thank you for your interest in catering services at ${restaurantName}!`;
  
  // Get catering link
  const cateringLink = restaurantData.cateringLink;
  
  // Add catering information section if link exists
  if (cateringLink) {
    message += ` To view our catering menu and place an order, please visit us at ${cateringLink}`;
  } else {
    // Fallback if no catering link available
    message += ` Please call us directly to inquire about catering options.`;
  }
...
// Function to send catering links via SMS
module.exports.sendCateringLink = async (event) => {
  console.log('Processing catering link SMS request...');
  ...
  try {
    ...
    console.log(`Processing catering links for location: ${locationId}, customer: ${customerPhone}`);
    ...
    const dynamicMessage = buildCateringMessage(restaurantData);

    // Send SMS with dynamic message
    const smsResult = await sendCateringLinkSMS(customerPhone, dynamicMessage, restaurantData.restaurantName);
    console.log('✅ Dynamic catering links SMS sent successfully');

    return createSuccessResponse({
      success: true,
      message: 'Catering links sent successfully',
      restaurantName: restaurantData.restaurantName,
      smsResult: smsResult
    });

  } catch (error) {
    console.error('Error in catering link workflow:', error.message);
...
// Function to send catering links SMS using TextBelt
async function sendCateringLinkSMS(phoneNumber, message, restaurantName) {
